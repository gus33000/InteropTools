        ...
        public async Task<UnlockStates> UnlockSSHSystemAccess()
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    Stream cmd = await (await StorageFile.GetFileFromApplicationUriAsync(new Uri("ms-appx:///SSH//cmd.exe", UriKind.Absolute))).OpenStreamForReadAsync();
                    Stream cmdmui = await (await StorageFile.GetFileFromApplicationUriAsync(new Uri("ms-appx:///SSH//en-US//cmd.exe.mui", UriKind.Absolute))).OpenStreamForReadAsync();
                    Stream reg = await (await StorageFile.GetFileFromApplicationUriAsync(new Uri("ms-appx:///SSH//reg.exe", UriKind.Absolute))).OpenStreamForReadAsync();
                    Stream regmui = await (await StorageFile.GetFileFromApplicationUriAsync(new Uri("ms-appx:///SSH//en-US//reg.exe.mui", UriKind.Absolute))).OpenStreamForReadAsync();
                    Stream netisol = await (await StorageFile.GetFileFromApplicationUriAsync(new Uri("ms-appx:///SSH//CheckNetIsolation.exe", UriKind.Absolute))).OpenStreamForReadAsync();
                    Stream netisolmui = await (await StorageFile.GetFileFromApplicationUriAsync(new Uri("ms-appx:///SSH//en-US//CheckNetIsolation.exe.mui", UriKind.Absolute))).OpenStreamForReadAsync();
