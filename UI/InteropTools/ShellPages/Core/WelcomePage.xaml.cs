...
namespace InteropTools.ShellPages.Core
{
	public sealed partial class WelcomePage
	{
		...
		{
			var item = (NavigationItem) ((StackPanel) sender).DataContext;
			CreateTile(item);
		}

		private async void CreateTile(NavigationItem item)
		{
			var page = item.PageType.Name;
			...
			var tileActivationArguments = page;
            var secondaryTile = new SecondaryTile(page,
                                                  item.DisplayName,
		...
		{
			var item = (NavigationItem) ((Grid) sender).DataContext;
			CreateTile(item);
		}
