...
namespace InteropTools.ShellPages.IO
{
    ...
    {
        //private readonly IRegistryProvider _helper = App.Helper;
        ...
        private void ShowEditValueDialog(RegistryItemCustom edititem, bool refresh)
        {
            ...
            uint regtype;
            string regvalue;
            _helper.GetKeyValue(currentEditItem.Hive, currentEditItem.Key ?? "", currentEditItem.Name, currentEditItem.ValueType, out regtype, out regvalue);
        ...
        /*private async void AddKey(RegHives hive, string keypath)
        {
            ...
            var command = await new InteropTools.ContentDialogs.Core.DualMessageDialogContentDialog().ShowDualMessageDialog(title, content, ResourceManager.Current.MainResourceMap.GetValue("Resources/Add_the_key", ResourceContext.GetForCurrentView()).ValueAsString, ResourceManager.Current.MainResourceMap.GetValue("Resources/Don_t_add_the_key", ResourceContext.GetForCurrentView()).ValueAsString);

            if (command)
                RunInThreadPool(() =>
                {
                    var status = _helper.AddKey(hive, keypath);
                    RunInUiThread(() =>
                    {
        ...
        private void PathInput_LostFocus(object sender, RoutedEventArgs e)
        {
            ...
            if (result)
            {
                RunInThreadPool(() =>
                {
                    var status = _helper.GetKeyStatus(hive, keypath);
                    RunInUiThread(() =>
                    {
                        switch (status)
                        {
            ...
            BrowserControl.CurrentItemChangedEventArgs e)
        {
            ...
            if (e.newItem != null)
            {
                ...
                if (e.newItem.type != BrowserControl.FileItemType.FILE)
                {
                        BreadCrumbBarIcon.Text = "";

                        BreadCrumbItemsList.Add(new BreadCrumbControl.BreadCrumbItem() { DisplayName = _helper.GetFriendlyName(), ItemObject = null });
                        BreadCrumbItemsList.Add(new BreadCrumbControl.BreadCrumbItem() { DisplayName = e.newItem.Hive.ToString(), ItemObject = new BrowserControl.FileItem() { name = e.newItem.name.ToString(), Type = RegistryItemType.HIVE, Value = null, ValueType = 0 } });

                        var current = "";

                        foreach (var item in e.newItem.path.Split('\\'))
                        {
                ...
                default:
                    {
                        try
                        {
                            string s = Regex.Replace(str, ".{2}", "$0-").TrimEnd('-');
