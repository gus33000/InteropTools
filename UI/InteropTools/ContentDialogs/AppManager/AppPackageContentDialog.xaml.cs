...
namespace InteropTools.ContentDialogs.AppManager
{
	public sealed partial class AppPackageContentDialog : ContentDialog
	{
		...
		{
			var PackageMan = new PackageManager();

			try
			{
				var package = PackageMan.FindPackageForUser("", fullname);
				_package = package;
				...
				try
				{
					var arch = ResourceManager.Current.MainResourceMap.GetValue("Resources/Unknown", ResourceContext.GetForCurrentView()).ValueAsString;
				...
				try
				{
					var statustext = ResourceManager.Current.MainResourceMap.GetValue("Resources/Ok", ResourceContext.GetForCurrentView()).ValueAsString;
				...
				try
				{
					...
					foreach (var AppEntry in AppEntries)
					{
						...
						try
						{
							...
							logosize.Height = 48;
							logosize.Width = 48;
							var applogo = AppEntry.DisplayInfo.GetLogo(logosize);
							var bitmapImage = new BitmapImage();
							var ras = await applogo.OpenReadAsync();
		...
		private async void OpenApp_Click(object sender, RoutedEventArgs e)
		{
			try
			{
				var results = await _package.GetAppListEntriesAsync();
